...
import com.lucky.sweet.constants.Constant;
...
public class Tab extends RelativeLayout {
    ...
    //Tab按钮点击效果
    private TabButton.OnTabClickListener onTabClickListener = new TabButton.OnTabClickListener() {
     ...
     * @param dataLst
     */
    public void setDataList(ArrayList<TabButton.TabButtonData> dataLst) {
        ...
        if (tabButtons.length == dataLst.size()) {
            ...
            for (int i = 0; i < buttonsLayout.getChildCount(); i++) {
                if (buttonsLayout.getChildAt(i) instanceof TabButton) {
                    ...
                    tabButtons[tabPosition].setOnTabClickListener(onTabClickListener);
    ...
    public interface OnTabChangeListener {
        void onTabChange(int position, String tag);
    }
    ...
    public TabButton getTabButton(String tag) {
        TabButton tabButton = null;
        if (tag.equals(Constant.TabTag.MAP)) {
            tabButton = tabButtons[0];
        } else if (tag.equals(Constant.TabTag.MSG)) {
            tabButton = tabButtons[1];
        } else if (tag.equals(Constant.TabTag.CONTRACT)) {
            tabButton = tabButtons[2];
        } else if (tag.equals(Constant.TabTag.ME)) {
            tabButton = tabButtons[3];
        }
        return tabButton;
    }
}
